...
inline void list_add_to_behind(struct List * entry, struct List * _new)	////add to entry behind
{
	_new->next = entry->next;
	_new->prev = entry;
	_new->next->prev = _new;
	entry->next = _new;
}

inline void list_add_to_before(struct List * entry,struct List * _new)	////add to entry behind
{
	_new->next = entry;
	entry->prev->next = _new;
	_new->prev = entry->prev;
	entry->prev = _new;
}
